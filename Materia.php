...
class Materia {
    ...
    public function datosMaestro($id_maestro){
        echo "<br><center>Maestro Seleccionado:</center>";
        $sql04 = "SELECT * FROM usuario WHERE id = $id_maestro";
        $result04 = mysql_query($sql04)or die("No se ejecuta consulta $sql04");
        $existe = mysql_num_rows($result04);
        echo"<div class=table-responsive>";
        echo"<table class=\"table table-striped\">";
        echo"<tr><td colspan='5' align='center'><strong>Datos del Maestro</strong></td></tr>";
        echo"<tr><td>ID</td><td>Nombre</td><td>Apellido Paterno</td><td>Apellido Materno</td><td>Estatus</td></tr>";
        if ($existe > 0) {
            $id= mysql_result($result04, 0, 'id');
            $nombre=mysql_result($result04, 0, 'Nombre');
            $app=mysql_result($result04, 0, 'ApellidoPaterno');
            $apm=mysql_result($result04, 0, 'ApellidoMaterno');
            $estatus=mysql_result($result04, 0, 'Estatus');

            echo"<tr><td>$id</td><td>$nombre</td><td>$app</td><td>$apm</td><td>$estatus</td></tr>";
        }
        echo"</table>";
        echo"</div>";
    }
